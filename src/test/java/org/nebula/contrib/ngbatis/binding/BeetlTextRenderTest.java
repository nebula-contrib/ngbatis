...
class BeetlTextRenderTest {
    ...
    public void test() {
        String text = "        MATCH (n:${ tag })\n" + "        WHERE\n"
                ...
                + "               n.${ tag }.${ col } == $${ col }\n"
                + "           @}\n" + "        @}\n" + "        LIMIT 4000";
        String cql = render.resolve(text, new HashMap<String, Object>() {
            {
    ...
    public void textMapFor() {
        ...
        String cql = render.resolve(text, new HashMap<String, Object>() {
            {
                put("p", new HashMap<String, Object>() {
                    {
                        put("name", "张三");
                        put("gender", "");
                    }
    ...
    public void textListFor() {
        String text = "            @for ( p in personList ) {\n"
                + "              INSERT VERTEX `person` ( name, gender ) VALUES '${ p.name }' : ( '${ p.name }', '${ p.gender }' );\n"
                + "            @}";

        List<HashMap<String, Object>> personList = Arrays
    ...
    public void textForBatchInsert() {
        String text = "  INSERT VERTEX ${tagName}\n" + "   (\n"
                + "      @for ( param in params ) {\n"
                + "          ${param} ${ paramLP.last ? '' : ',' }\n"
                + "      @}\n" + "   )\n" + "  VALUES\n"

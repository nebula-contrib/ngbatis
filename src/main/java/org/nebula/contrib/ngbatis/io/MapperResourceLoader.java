...
public class MapperResourceLoader extends PathMatchingResourcePatternResolver {
    ...
    private static Logger log = LoggerFactory
            .getLogger(MapperResourceLoader.class);
    ...
    public Map<String, ClassModel> load() {
        ...
        try {
            Resource[] resources = getResources(
                    parseConfig.getMapperLocations());
     ...
     */
    private Map<String, MethodModel> parseMethodModel(Class namespace,
            List<Node> nodes) {
        ...
        for (Node methodNode : nodes) {
            if (methodNode instanceof Element) {
                ...
                Method method = getNameUniqueMethod(namespace,
                        methodModel.getId());
                methodModel.setMethod(method);
                Assert.notNull(method, "接口 " + namespace.getName()
     ...
     */
    private void pageSupport(Method method, MethodModel methodModel,
            List<String> methodNames, Map<String, MethodModel> methods) {
        ...
        if (parameterTypeList.contains(Page.class)) {
            ...
            MethodModel pageMethod = createPageMethod(methodModel, methodNames,
                    parameterTypes, pageParamIndex);
            methods.put(pageMethod.getId(), pageMethod);

            MethodModel countMethod = createCountMethod(methodModel,
                    methodNames, parameterTypes);
     ...
     * @return
     */
    private MethodModel createCountMethod(MethodModel methodModel,
            List<String> methodNames, Class<?>[] parameterTypes) {
     ...
     */
    private MethodModel createPageMethod(MethodModel methodModel,
            List<String> methodNames, Class<?>[] parameterTypes,
            int pageParamIndex) {
        String methodName = methodModel.getId();
    ...
    private void checkReturnType(Method method, Class namespace) {
        ...
        if (NEED_SEALING_TYPES.contains(returnType)) {
            throw new ResourceLoadException("目前不支持返回基本类型，请使用对应的包装类，接口："

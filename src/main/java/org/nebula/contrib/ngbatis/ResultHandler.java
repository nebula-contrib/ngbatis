...
public interface ResultHandler<T, Z> {
    ...
    static ResultHandler<Object, Object> getHandler(
        ...
        if (classResultHandler == null) {
            ...
            for (Class itf : interfaces) {
                ResultHandler<Object, Object> handler = getHandler(handlerMap,
                        itf);
                if (handler != null) {
                    return handler;
                }
    ...
    static ResultHandler<Object, Object> getHandler(Class returnType,
            Class resultType) {
        Map<Class, ResultHandler> classResultHandlerMap = DIFF_TYPE_RESULT_HANDLER
                .get(returnType);
        ...
        if (classResultHandler == null) {
            Class[] interfaces = returnType.getInterfaces();
            for (Class itf : interfaces) {
                ResultHandler<Object, Object> handler = getHandler(itf,
                        resultType);
                if (handler != null) {
                    return handler;
                }

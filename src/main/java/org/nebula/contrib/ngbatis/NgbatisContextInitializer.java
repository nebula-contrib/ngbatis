...
public class NgbatisContextInitializer
    ...
    public void initialize(ConfigurableApplicationContext context) {
        ...
        NebulaJdbcProperties nebulaJdbcProperties = getNebulaJdbcProperties(
                environment).setPoolConfig(nebulaPool);
    ...
    private ParseCfgProps readParseCfgProps(
        ...
        return new ParseCfgProps()
                ...
                .setLogShow(environment.getProperty(
                        "cql.parser.log-show"))
                .setMapper(environment.getProperty(
                        "cql.parser.mapper"))
                .setNamespace(environment.getProperty(
                        "cql.parser.namespace"))
                ...
                .setResultType(environment.getProperty(
                        "cql.parser.result-type"))
                .setParameterType(environment.getProperty(
                        "cql.parser.parameter-type"))
                .setStatementEnd(environment.getProperty(
                        "cql.parser.statement-end"))
    ...
    private NebulaJdbcProperties getNebulaJdbcProperties(
        ...
        return nebulaJdbcProperties
                .setHosts(Objects.requireNonNull(environment
                        .getProperty("nebula.hosts")))
                .setUsername(environment
                        .getProperty("nebula.username"))
                .setPassword(environment
                        .getProperty("nebula.password"))
                .setSpace(environment
                        .getProperty("nebula.space"));
    }

    private NebulaPoolConfig getNebulaPoolConfig(
            ConfigurableEnvironment environment) {
        NebulaPoolConfig nebulaPoolConfig = new NebulaPoolConfig()
                .setMinConnSize(environment.getProperty(
                        "nebula.pool-config.min-conn-size",
                        Integer.class, 0))
                .setMaxConnSize(environment.getProperty(
                        "nebula.pool-config.max-conn-size",
                        Integer.class, 10))
                .setTimeout(environment.getProperty(
                        "nebula.pool-config.timeout",
                        Integer.class, 0))
                .setIdleTime(environment.getProperty(
                        "nebula.pool-config.idle-time",
                        Integer.class, 0))
                .setIntervalIdle(environment.getProperty(
                        "nebula.pool-config.interval-idle",
                        Integer.class, -1))
                .setWaitTime(environment.getProperty(
                        "nebula.pool-config.wait-time",
                        Integer.class, 0));
...
class NgbatisBeanFactoryPostProcessor
    ...
    private Logger log = LoggerFactory
            .getLogger(NgbatisBeanFactoryPostProcessor.class);
    ...
    public MapperContext mapperContext(NebulaPool nebulaPool) {
        ...
        Map<String, ClassModel> interfaces = daoBasicResourceLoader
                .load();
        Map<String, String> daoBasicTpl = daoBasicResourceLoader
                .loadTpl();
        context.setDaoBasicTpl(daoBasicTpl);
        context.setNebulaPool(nebulaPool);
        context.setInterfaces(interfaces);
        context.setNebulaPoolConfig(
                nebulaJdbcProperties.getPoolConfig());
        figureTagTypeMapping(interfaces.values(),
                context.getTagTypeMapping());
     ...
     */
    private void figureTagTypeMapping(Collection<ClassModel> classModels,
        ...
        for (ClassModel classModel : classModels) {
            ...
            if (entityTypeAndIdType != null) {
                ...
                tagTypeMapping.putIfAbsent(vertexName,
                        entityType);
            }
        }
        ...
        for (ClassModel cm : interfaces.values()) {
            try {
                String className = cm.getNamespace().getName()
                        + PROXY_SUFFIX;
                registerBean(cm, ramClassLoader
                        .loadClass(className));
                log.info("Bean had been registed  (代理类注册成bean): {}",
                        className);
    ...
    private String getBeanName(ClassModel cm) {
        Annotation annotation = cm.getNamespace()
    ...
    public NebulaPool nebulaPool() {
        ...
        try {
            pool.init(nebulaJdbcProperties.getHostAddresses(),
                    nebulaJdbcProperties.getPoolConfig());
        } catch (UnknownHostException e) {
            throw new RuntimeException(
                    "Can not connect to Nebula Graph");
        }
        return pool;
    }

...
public abstract class GraphBaseEdge extends GraphBase {
	 ...
	 * @param <T> 起点类型
	 */
	public <I extends GraphBaseEdge,T extends GraphBaseVertex> List<I> queryByProperty(T srcVertex,Direction direction,T dstVertex){
		String typeName = getEntityTypeName();
		Map<String, Object> entityProperties = getEntityProperties();
		String gql = GraphQueryBuilder.matchEdgeWithEdgeProperty(srcVertex,dstVertex,typeName,entityProperties,direction);
		log.info("nGQL:{}",gql);
		ResultSet resultSet = null;
		try {
			resultSet = executeGQL(gql);

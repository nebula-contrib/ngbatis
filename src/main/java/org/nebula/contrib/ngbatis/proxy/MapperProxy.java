...
public class MapperProxy {
    ...
    public static Object invoke(String className, String methodName,
        ...
        ClassModel classModel = mapperContext.getInterfaces()
                .get(proxyClassName);
        Method method = null;
        if (mapperContext.isResourceRefresh()) {
            try {
                Map<String, ClassModel> classModelMap = classModel
                        .getResourceLoader()
                        .parseClassModel(classModel.getResource());
     ...
     * @return 结果对象映射的 java 对象
     */
    private static Object pageSupport(ClassModel classModel, Method method,
            Object[] args) {
        ...
        Long count = (Long) mapperProxy.invoke(classModel,
                classModel.getMethods().get(countMethodName), args);
        List rows = (List) mapperProxy.invoke(classModel,
                classModel.getMethods().get(pageMethodName), args);
    ...
    public static Object invoke(ClassModel classModel, MethodModel methodModel,
        ...
        Map<String, Object> argMap = ENV.getArgsResolver().resolve(methodModel,
                args);
        Map<String, Object> paramWithSchema = new LinkedHashMap<String, Object>(
                argMap) {
            {
        ...
        if (method != null
                && method.isAnnotationPresent(UseKeyArgReplace.class)) {
            ArgNameFormatter.CqlAndArgs format = ENV.getArgNameFormatter()
                    .format(nGQL, argMap);
            nGQL = format.getCql();
     ...
     */
    public static ResultSet executeWithParameter(ClassModel cm, MethodModel mm,
            String nGQL, Map<String, Object> params) {
        ...
        try {
            ...
            log.debug(
                    "\n\t- proxyMethod: {}#{} \n\t- nGql：{} \n\t - params: {}\n\t - result：{}",
                    proxyClass, proxyMethod, nGQL, params, result);

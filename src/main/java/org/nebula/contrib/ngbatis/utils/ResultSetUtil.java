...
public class ResultSetUtil {
    ...
    private static Object transformDateTime(DateTimeWrapper dateTime) {
        return new GregorianCalendar(dateTime.getYear(),
                dateTime.getMonth() - 1, dateTime.getDay(), dateTime.getHour(),
                dateTime.getMinute(), dateTime.getSecond()).getTime();
    }

    private static Object transformDate(DateWrapper date) {
        return new GregorianCalendar(date.getYear() + 1900, date.getMonth() - 1,
                date.getDay()).getTime();
    }
    ...
    private static Object transformNode(Node node) {
        ...
        Map<String, Class<?>> tagTypeMapping = mapperContext
                .getTagTypeMapping();
    ...
    private static Object transformList(ArrayList<ValueWrapper> list) {
        return list.stream().map(ResultSetUtil::getValue)
    ...
    public static <T> T nodeToResultType(Node v, Class<T> resultType) {
        ...
        try {
            ...
            for (int i = 0; i < keys.size(); i++) {
                ...
                ReflectUtil.setValue(t, prop,
                        ResultSetUtil.getValue(values.get(i)));
            }
            setId(t, resultType, v);
        } catch (UnsupportedEncodingException | InstantiationException
                | IllegalAccessException | NoSuchFieldException e) {
            throw new ResultHandleException(String.format("%s : %s",
    ...
    public static <T> T relationshipToResultType(Relationship r,
            Class<T> resultType) {
        T t = null;
        try {
            t = resultType.newInstance();
            HashMap<String, ValueWrapper> properties = r.properties();
            for (Map.Entry<String, ValueWrapper> entry : properties
                    .entrySet()) {
                ReflectUtil.setValue(t, entry.getKey(),
                        ResultSetUtil.getValue(entry.getValue()));
            }
        } catch (UnsupportedEncodingException | InstantiationException
    ...
    public static void relationshipToResultType(Object o, String fieldName,
        ...
        Class<?> fieldType = ReflectUtil.fieldType(o, fieldName);
        if (fieldType != null) {
            Object fieldValue = relationshipToResultType(relationship,
                    fieldType);
            try {
                ReflectUtil.setValue(o, fieldName, fieldValue);
            } catch (NoSuchFieldException | IllegalAccessException e) {

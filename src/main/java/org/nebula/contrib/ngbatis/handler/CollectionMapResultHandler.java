 ...
 */
@Component
public class CollectionMapResultHandler
        extends AbstractResultHandler<Collection, Map> {
    ...
    public Collection handle(Collection newResult, ResultSet result,
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < columnNames.size(); j++) {
                ...
                row.put(columnNames.get(j),
                        ResultSetUtil.getValue(valueWrapper));
            }
            newResult.add(row);
        }
        return newResult;
